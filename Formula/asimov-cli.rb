class AsimovCli < Formula
  ...
  def install
    # Decompress the downloaded .gz file first
    system "gunzip", cached_download

    # The uncompressed file has the same name minus ".gz"
    uncompressed = Pathname(cached_download.to_s.sub(/\.gz$/, ""))

    # Install it as "asimov" in Homebrew's bin directory
    bin.install uncompressed => "asimov"
  end

  test do
    # Basic check that the CLI runs
